	...
	function praseContent($res,$fname,$ftype)
	{
		...
		foreach($res as $item)
		{
			...
			for($i;$i<sizeof($fname);$i++)
			{
				...
				{
					if($item[$fname[$i]['name']]=="")
					{
						$this->_res[$this->_index]['content'].="<td><img height='30' src='".base_url("images/defaultimg.jpg")."'/></td>";
					}
					else
					{
						$this->_res[$this->_index]['content'].="<td><a href='".base_url($item[$fname[$i]['name']])."' target='_blank'><img height='30' src='".base_url()."'/></a></td>";
					}
				}
				else
				{
